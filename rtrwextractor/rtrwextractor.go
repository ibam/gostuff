...
var rwRegex = regexp.MustCompile("(?i)RW(\\W*\\d+)*")
...
func main() {
	...
	headersWithRTRW := append(headers, "lokasi_rt", "lokasi_rw")
	csvWriter.Write(headersWithRTRW)
	...
	for _, record := range records {
		...
		rwCandidates := extractFromMultipleCandidates(rwRegex, alamatLokasi, masalah, usulanSolusi)

		rtCounter += len(rtCandidates)
		rwCounter += len(rwCandidates)

		if len(rtCandidates) > 0 || len(rwCandidates) > 0 {
			fmt.Println("\nAlamat:", alamatLokasi)
			fmt.Println("Masalah:", masalah)
			fmt.Println("Solusi:", usulanSolusi)
			fmt.Println("Found RT:", rtCandidates)
			fmt.Println("Found RW:", rwCandidates)
		}

		record = append(record, strings.Join(rtCandidates, ", "))
		record = append(record, strings.Join(rwCandidates, ", "))
		csvWriter.Write(record)
	}

	fmt.Println("Finished processing", len(records), "records, found", rtCounter, "RT and", rwCounter, "RW")
